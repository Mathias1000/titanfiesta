...
protected:
	void ReceivedClientPacket(char* buffer, dword length){
		...
		pak.GetFromBuffer();
		if(srvType == 0){
			if(!ReceivedLoginClientPacket(&pak, this)) return;
		}else if(srvType == 1){
			if(!ReceivedWorldClientPacket(&pak, this)) return;
		}else if(srvType == 2){
			if(!ReceivedGameClientPacket(&pak, this)) return;
		}
	...
	void ReceivedServerPacket(char* buffer, dword length){
		...
		if(srvType == 0){
			if(!ReceivedLoginServerPacket(&pak, this)) return;
		}else if(srvType == 1){
			if(!ReceivedWorldServerPacket(&pak, this)) return;
		}else if(srvType == 2){
			if(!ReceivedGameServerPacket(&pak, this)) return;
		}
