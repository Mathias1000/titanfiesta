...
public:
	...
	bool Launch(char* username, char* md5hash){
		...
		char url[255];
		sprintf_s(url, 255, "http://rest.outspark.net/user/v1/login?realm=fiesta&user=%s&password=%s&version=520&output=text", username, md5hash);
	...
	void LaunchExe(char* token){
		...
		sprintf_s(commandLine, 256, "Fiesta.bin -osk_token %s -osk_server 127.0.0.1 -osk_store http://titanfiesta.wikidot.com/", token);
