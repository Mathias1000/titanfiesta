/* Copyright (C) 2008, 2009 TitanFiesta Dev Team
 * Licensed under GNU GPL v3
 * For license details, see LICENCE in the root folder. */

static const unsigned char xorTable[499] =
{
	, 
};

class CFiestaCrypt {
public:
	CFiestaCrypt(){}
	~CFiestaCrypt(){}

	void SetCryptLocation(int newLocation){
		cryptStatus = newLocation;
	}

	void CryptPacket(CPacket* pak, bool moveLocation = true){
		boost::mutex::scoped_lock lCrypt(mCrypt);
		
		word start = (pak->buffer[0] == 0)?3:1;
		int cryptLoc = cryptStatus;
		for(word i = start; i < pak->size; ++i){
			pak->buffer[i] = pak->buffer[i] ^ xorTable[cryptLoc];
			cryptLoc++;
			if(cryptLoc == 0x1F3) cryptLoc = 0;
		}
		if(moveLocation) cryptStatus = cryptLoc;

		lCrypt.unlock();
	}

private:
	int cryptStatus;
	boost::mutex mCrypt;
};
